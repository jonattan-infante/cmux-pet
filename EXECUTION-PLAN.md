...
| F4 | Responder permiso/pregunta desde la burbuja (PR2 de 2, `docs/adr/0009`): botones en la UI | plomería (PR1) entregada; falta `BubbleView` con sub-regiones clicables y el wiring en `PetController+Bubbles.swift` | agregar los botones a `BubbleView`, probar a mano contra un permiso/pregunta real y cerrar el ⚠️ de `docs/reference/agent-reply.md` (que `{"delivered":true}` mueva de verdad al agente) |
