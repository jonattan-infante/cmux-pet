# Hook de Claude Code para cmux-pet en Windows.
#
...
# ese payload a una linea compacta y la agrega a %USERPROFILE%\.cmux-pet\shell.jsonl,
...
#   powershell -NoProfile -ExecutionPolicy Bypass -File cmux-pet-hook.ps1
    ...
    if ($evName -eq "SessionStart") {
        try {
            ...
            if ((-not $alive) -and (Test-Path $petScript)) {
                ...
                if ($py -and (-not $env:CMUX_PET_NO_AUTOSTART)) {
