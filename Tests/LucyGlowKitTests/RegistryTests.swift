...
final class RegistryTests: XCTestCase {
    ...
    func testRegistroConfigurablePorEntorno() {
        let original = ProcessInfo.processInfo.environment["LUCY_REGISTRY"]
        setenv("LUCY_REGISTRY", "https://ejemplo.interno/pets.json", 1)
        XCTAssertEqual(Registry.url.absoluteString, "https://ejemplo.interno/pets.json")
        if let o = original { setenv("LUCY_REGISTRY", o, 1) } else { unsetenv("LUCY_REGISTRY") }
    ...
    func testElRegistroDelRepoEsValido() throws {
        let repo = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()   // LucyGlowKitTests
    ...
    /// esto, `lucy install <id>` fallaria despues del clon.
    func testLasMascotasDelRepoCargan() throws {
        let repo = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent()
        let data = try Data(contentsOf: repo.appendingPathComponent("registry.json"))
        let entries = Registry.parse(data) ?? []
        for e in entries where e.source.contains("jonattan-infante/lucyglow") {
