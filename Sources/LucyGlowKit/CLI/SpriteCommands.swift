...
extension PetCommands {
    ...
    /// `lucy sprite <id> <estado> <archivo>` pone una imagen.
    /// `lucy sprite <id> --dir <carpeta>` toma varias por nombre de archivo.
    /// `lucy sprite <id> --clear` vuelve al dibujo vectorial.
    static func sprite(_ args: [String]) -> Int32 {
        let positional = args.filter { !$0.hasPrefix("--") }
        guard let id = positional.first else {
            err("falta el id. Uso: lucy sprite <id> <estado> <archivo>")
            err("                 lucy sprite <id> --dir <carpeta>")
            err("                 lucy sprite <id> --clear")
            return 2
        }
        ...
        if PetLibrary.isBundled(id) {
            err("\"\(id)\" viene con lucy y se reemplaza al actualizar.")
            err("Saca tu propia copia y edítala:")
            err("    lucy fork \(id) mi-\(id)")
            err("    lucy sprite mi-\(id) \(positional.count > 1 ? positional[1] : "idle") <archivo>")
            return 1
        }
        ...
        guard positional.count >= 3 else {
            err("faltan argumentos. Uso: lucy sprite \(id) <estado> <archivo>")
    ...
    static func fork(_ args: [String]) -> Int32 {
        ...
        guard positional.count >= 2 else {
            err("Uso: lucy fork <origen> <nuevo-id> [--name <texto>]")
            return 2
        }
        ...
        let code = updateManifest(copy) { m in
            ...
            out("  2. lucy sprite \(newID) idle <imagen.png>   " + dim("o --dir <carpeta>"))
            out("  3. lucy use \(newID)")
            out("  4. lucy voice \(newID)          " + dim("frases con su personalidad"))
        }
        return code
    }
