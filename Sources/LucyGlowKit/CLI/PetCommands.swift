...
public enum PetCommands {
    ...
    static func help() -> Int32 {
        out("""
        \(bold("lucy")) — LucyGlow, la mascota flotante que vigila tus agentes de IA

        \(bold("Uso"))
          lucy                        arranca la mascota activa
          lucy <comando> [opciones]
    ...
    static func list() -> Int32 {
        ...
        guard !pets.isEmpty else {
            ...
            out("")
            out("  lucy search           ver el marketplace")
            out("  lucy install astro    instalar la de por defecto")
            out("  lucy new mi-mascota   crear una propia")
            return 0
        }
        ...
        out("")
        out(dim("  * activa. Cambiar con: lucy use <id>"))
        return 0
    }
    ...
    static func use(_ args: [String]) -> Int32 {
        guard let id = args.first else {
            err("falta el id. Uso: lucy use <id>")
            return 2
        }
        guard let pack = PetLibrary.find(id) else {
            err("no tengo instalada \"\(id)\". Mira \"lucy list\".")
            return 1
        }
    ...
    static func info(_ args: [String]) -> Int32 {
        guard let id = args.first else {
            err("falta el id. Uso: lucy info <id>")
            return 2
        }
    ...
    static func install(_ args: [String]) -> Int32 {
        ...
        guard let target = args.first(where: { !$0.hasPrefix("--") }) else {
            err("falta qué instalar. Uso: lucy install <id|url-git|ruta>")
            return 2
        }
    ...
    static func installFromGit(url: String, subpath: String?, force: Bool, use: Bool) -> Int32 {
        let tmp = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("lucy-install-\(getpid())")
    ...
    static func finishInstall(_ result: Result<PetPack, PetLibrary.InstallError>, use: Bool) -> Int32 {
        ...
        case .success(let p):
            ...
            out(dim("  activar con: lucy use \(p.id)"))
            return 0
        }
    }
    ...
    static func uninstall(_ args: [String]) -> Int32 {
        guard let id = args.first else {
            err("falta el id. Uso: lucy uninstall <id>")
            return 2
        }
        ...
        if config.activePet == id {
            ...
            if let next = config.activePet {
                ...
                out(dim("  no queda ninguna instalada; instala otra con: lucy install astro"))
            }
        }
        return 0
    }
    ...
    static func scaffold(_ args: [String]) -> Int32 {
        guard let id = args.first(where: { !$0.hasPrefix("--") }) else {
            err("falta el id. Uso: lucy new <id> [--sprites] [--name <texto>]")
            return 2
        }
        ...
        out("  3. lucy validate ./\(id)")
        out("  4. lucy install ./\(id) --use")
        out("  5. lucy voice \(id)      " + dim("Claude Code le escribe las frases"))
        out("")
    ...
    static func renderers() -> Int32 {
        ...
        out("  \(dim("                lucy sprite <id> <estado> <archivo>"))")
        out("")
    ...
    static func validate(_ args: [String]) -> Int32 {
        ...
        out("")
        if problems == 0 {
            out("Sin problemas. Instalar con: lucy install \(target) --use")
            return 0
        }
    ...
    static func voice(_ args: [String]) -> Int32 {
        ...
        if let id = id {
            ...
            if pack == nil { err("no hay mascota activa. Instala una: lucy install astro"); return 1 }
        }
    ...
    static func search(_ args: [String]) -> Int32 {
        ...
        out("")
        out(dim("  * ya instalada.  Instalar: lucy install <id> --use"))
        return 0
    }
    ...
    static let installerURL =
        "https://raw.githubusercontent.com/jonattan-infante/lucyglow/main/install.sh"
    ...
    static func update(_ args: [String]) -> Int32 {
        guard let current = Semver(lucyGlowVersion) else { return 1 }
        out("lucy \(current)")
        ...
        if let latest = latest, latest <= current, !args.contains("--force") {
            out(dim("  para reinstalar igual: lucy update --force"))
            return 0
        }
    ...
    static func restartRunningApp() -> Bool {
        ...
        let binary = PetPaths.home.appendingPathComponent("bin/lucy")
        kill(pid, SIGTERM)
        usleep(500_000)
