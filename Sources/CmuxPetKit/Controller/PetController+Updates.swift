...
extension PetController {
    ...
    func maybeCheckForUpdate() {
        ...
        guard let current = Semver(cmuxPetVersion) else { return }
    ...
    func announceUpdate(_ v: Semver) {
        plog("actualizaciones: hay \(v), corre \(cmuxPetVersion)")
        show(Bubble(mood: .info,
                    text: Voice.shared.phrase("updateAvailable", ["version": v.description])
                        ?? Wording.plain("Hay una versión nueva de cmux-pet: \(v). Corre: cmux-pet update"),
